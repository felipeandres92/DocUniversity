...
var PasswordsLib = function(){
  ...
  self.create = function(passwordData, callback){
    ...
    PasswordModel.findOne({ username: newPassword.username }, function(err, pass) {
        if (err) throw err;
     
        // test a matching password
        pass.comparePassword(newPassword.password, function(err, isMatch) {
            if (err) throw err;
            console.log(newPassword.password, isMatch); // -&gt; Password123: true
        });
     
        // test a failing password
        pass.comparePassword(newPassword.password, function(err, isMatch) {
            if (err) throw err;
            console.log(newPassword.password, isMatch); // -&gt; 123Password: false
        });
      });
  };
